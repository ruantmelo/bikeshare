...
export default async function bikeRoutes(app) {
  ...
  app.get('/admin/:bikeId/telemetry', { preHandler: adminOnly }, async (request) => {
  ...
  app.delete('/:bikeId', { preHandler: adminOnly }, async (request, reply) => {
    const { bikeId } = request.params
